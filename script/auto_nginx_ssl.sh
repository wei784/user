...
# 使用 printf 替代 echo -e 以获得更好的可移植性
print_info() {
    printf '%b\n' "${BLUE}[INFO] $1${NC}"
}
...
install_dependencies() {
    ...
    packages_to_install=""
    required_commands="nginx certbot curl"
...
get_user_input() {
    ...
    # 获取域名
    while true; do
        printf "请输入您的域名 (例如: sub.yourdomain.com): "
        read -r DOMAIN
        # 使用 grep -E 进行正则匹配
    ...
    # 获取反向代理目标地址
    while true; do
        printf "请选择反向代理目标的协议 [http/https] (默认: http): "
        read -r protocol
    ...
    while true; do
        printf "请输入反向代理的目标地址 (例如: 127.0.0.1:8080 或仅输入端口 8080): "
        read -r address
        if [ -n "$address" ]; then
            if echo "$address" | grep -qE '^[0-9]+$'; then
    ...
    # 获取邮箱
    while true; do
        printf "请输入您的邮箱地址 (用于 Let's Encrypt 证书通知): "
        read -r EMAIL
    ...
    # 询问是否以测试模式运行
    TEST_MODE="no"
    printf "是否以测试模式运行 Certbot (dry-run)? [y/N] (默认: N): "
    read -r test_choice
    case "$test_choice" in
        [Yy]) 
            TEST_MODE="yes"
...
main() {
    clear
    printf '%b\n' "${GREEN}=====================================================${NC}"
    printf '%b\n' "${GREEN}  Nginx & Certbot 一键反代与 SSL 证书自动化脚本  ${NC}"
    printf '%b\n' "${GREEN}=====================================================${NC}"
    printf '\n'
